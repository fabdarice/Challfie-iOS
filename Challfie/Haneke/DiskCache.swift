    ...
    public class DiskCache {
                ...
                if let contents = fileManager.contentsOfDirectoryAtPath(cachePath, error: &error) as? [String] {
                    for pathComponent in contents {
                        ...
                        if !fileManager.removeItemAtPath(path, error: &error) {
                            Haneke.Log.error("Failed to remove path \(path)", error)
                        }
                    }
                    self.calculateSize()
                } else {
            ...
            if let contents = fileManager.contentsOfDirectoryAtPath(cachePath, error: &error) as? [String] {
                for pathComponent in contents {
                    ...
                    if let attributes : NSDictionary = fileManager.attributesOfItemAtPath(path, error: &error) {
                        ...
                        Haneke.Log.error("Failed to read file size of \(path)", error)
                    }
                }
            } else {
                Haneke.Log.error("Failed to list directory", error)
        ...
        private func setDataSync(getData : @autoclosure () -> NSData?, key : String) {
            ...
            if let data = getData() {
                ...
                if (!success) {
                    Haneke.Log.error("Failed to write key \(key)", error)
                }
                ...
                Haneke.Log.error("Failed to get data for key \(key)")
            }
        }
        ...
        private func updateDiskAccessDateAtPath(path : String) -> Bool {
            ...
            if !success {
                Haneke.Log.error("Failed to update access date", error)
            }
            return success
        }
        ...
        private func removeFileAtPath(path:String) {
            ...
            if let attributes : NSDictionary = fileManager.attributesOfItemAtPath(path, error: &error) {
                ...
                if fileManager.removeItemAtPath(path, error: &error) {
                    ...
                    Haneke.Log.error("Failed to remove file", error)
                }
            } else {
                Haneke.Log.error("Failed to remove file", error)
            }
