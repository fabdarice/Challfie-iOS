...
- (CGRect)_calcVisibleRectForResizeableCropArea{
    ...
    return visibleRect = GKScaleRect(visibleRect, 100.1f);
}
...
- (void)layoutSubviews{
    ...
    scrollView.scrollEnabled = true; // Custom Fab
    
    if (width > height) {
        faktor = height / size.height; // CUSTOM FAB
        faktoredWidth = width / faktor; // CUSTOM FAB
        faktoredHeight =  size.height; // CUSTOM FAB
        scrollView.contentOffset = CGPointMake((faktoredWidth - size.width) / 2, 0); // Custom Fab
    } else {
        faktor = width / size.width; // CUSTOM FAB
        faktoredWidth = size.width; // CUSTOM FAB
